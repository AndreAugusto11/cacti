...
export class OracleUnregisterTaskEndpointV1 implements IWebServiceEndpoint {
  ...
  public async handleRequest(req: Request, res: Response): Promise<void> {
    ...
    const reqBody: OracleUnregisterRequest = req.body;
    this.log.debug("reqBody: ", reqBody);

    try {
      const result =
        await this.options.dispatcher.OracleUnregisterTask(reqBody);
      res.status(200).json(result);
    } catch (ex) {
      const errorMsg = `${reqTag} ${fnTag} Failed to unregister task: ${getEnumKeyByValue(SATPErrorType, (ex as SATPInternalError).getSATPErrorType())}`;
