...
beforeAll(async () => {
  ...
  apiServer = await gateway.getOrCreateHttpServer();
...
describe("Oracle executing READ, UPDATE, and READ_AND_UPDATE tasks successfully", () => {
  it("should fail when writing to a contract calling a function that does not exist", async () => {
    ...
    let response = await dispatcher.OracleExecuteTask({
    ...
    response = await dispatcher.OracleGetTaskStatus({
    // response = await oracleApi.getOracleTaskStatus({
      taskID: response.id ?? "",
    });
    ...
    expect(response.status).toBe(OracleTaskStatusEnum.Inactive);
  });
  ...
  it("should write data to a contract calling a function with args (EVM and Fabric)", async () => {
    ...
    response = await dispatcher.OracleExecuteTask({
  ...
  it("should read the data from the contract calling a function with args (EVM and Fabric)", async () => {
    ...
    const response2 = await dispatcher.OracleGetTaskStatus({
    ...
    expect(response2).toBeDefined();
    expect(response2?.status).toBe(OracleTaskStatusEnum.Inactive);
    expect(response2?.id).toBe(response.id);
    ...
    expect(response.operations?.[0]?.output?.output!).toBe("500");
  ...
  it("should read data and write it to another blockchain (EVM to Besu)", async () => {
    const response = await dispatcher.OracleExecuteTask({
    // const response = await oracleApi.executeOracleTask({
      sourceNetworkId: ethereumEnv.network,
      sourceContract: {
        contractName: ethereumEnv.getTestContractName(),
    ...
    expect(response.operations[0].type).toBe(OracleOperationTypeEnum.Read);
    expect(response.operations[1].type).toBe(
